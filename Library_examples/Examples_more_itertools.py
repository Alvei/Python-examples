...
def try_consecutive_group():
    """ we have a list of dates, where some of them are consecutive. To be able to pass these
    dates to consecutive_groups function, we first have to convert them to ordinal numbers.
    Then using list comprehension we iterate over groups of consecutive ordinal dates created by
    consecutive_groups and convert them back to datetime.datetime using map and fromordinal functions."""
...
def try_split_at():
    """ This works like basic split for strings, but here we have iterable instead of
...
def try_bucket():
    """ Split iterable into multiple buckets based on some condition.
          It creates child iterables by splitting input iterable using key function."""
    ...
    # Create a bucket object s, <more_itertools.more.bucket object at 0x7fa65323f210>
    s = bucket(shapes, key=lambda x: type(x))

    print(f"\nCube: {list(s[Cube])}")
    #  [<__main__.Cube object at 0x7f394a0633c8>, <__main__.Cube object at 0x7f394a063278>]
    print(f"\nCircle: {list(s[Circle])}")
...
def try_map_reduce():
    """ MapReduce implementation allows us to specify 3 functions:
        key function (for categorizing),
        value function (for transforming) and
        finally reduce function (for reducing)."""
...
def try_filter_except():
    """ Let’s look at following scenario: You received mixed data, that contains both text and numbers
        and all of it is in string form. You, however, want to work only with numbers (floats/ints) """
